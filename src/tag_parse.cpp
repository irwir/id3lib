...
namespace
{
  bool parseFrames(ID3_TagImpl& tag, ID3_Reader& rdr, bool bCompressed = false)
  {
    ...
    while (!rdr.atEnd() && rdr.peekChar() != '\0')
    {
      ...
      ID3_Reader::pos_type last_pos = rdr.getCur();
      ...
      else if (bCompressed)
      {
        ...
        ID3D_WARNING( "id3::v2::parseFrames(): illegal recursive compression");
        delete f;
      }
      else
      {
        ...
        // Uncompress and call parseFrames
        ...
        if (fld)
        {
          ...
          else
          {
            ...
            if (INVALID_DECOMPRESSION(mr.remainingBytes(), newSize))
            {
              // data corruption suspected, skip
              ID3D_WARNING( "id3::v2::parseFrames(): uncompressed size too big");
            }
            else
            {
              io::CompressedReader cr(mr, (uLong)newSize);
              parseFrames(tag, cr, true);
              if (!cr.atEnd())
              {
...
{ //changes in this routine should also be made in the routine for streaming parsing below
  ...
  ID3_Reader::pos_type en  = wr.getEnd();
