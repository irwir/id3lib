...
*/
    ...
    if (extflagbytes > 0) {
      ID3_Flags extflags; // ID3V2_4_0 has 1 flag byte, extflagbytes should be equal to 1,
      //other flag bytes will be skipped without parsing
      for (int i = 0; i < extflagbytes; ++i)
      {
        const ID3_Reader::int_type Ch = reader.readChar();
        if (!i)
          extflags.set(Ch); //flags
      }
      ...
      if (extflags.test(EXT_HEADER_FLAG_BIT2))
      {
        ...
        const int16 extheaderflagdatasize = reader.readChar();
